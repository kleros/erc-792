...
There will be two scenarios:
 ...
 2. ``payer`` reclaims funds by depositing arbitration fee...
      a. ``payee`` fails to deposit arbitration fee in ``arbitrationFeeDepositPeriod`` and ``payer`` wins by default. The arbitration fee deposit refunded.
      b. ``payee`` deposits arbitration fee in time. Dispute gets created. ``arbitrator`` rules. Winner gets the arbitration fee refunded.

So in effect, the loser will be paying for the arbitration.
...
.. literalinclude:: ../contracts/examples/SimpleEscrow.sol
    :language: javascript
    :lines: 1-19,24-40
    :emphasize-lines: 17,18,28-36


In ``releaseFunds`` function, first we do state checks, ``reclamationPeriod`` should be passed unless the caller is ``payer`` and transaction should be in ``Status.Initial``.
