...
In ``SimpleEscrow`` contract, we used a ``string`` to store the agreement between parties. Deployer could format that ``string`` anyway they like as
there are many ways to signal the information about the agreement.

Having a standard would allow interoperability. That's why *ERC-1497: Evidence Standard* describes a standard approach for this. It has two categories of information: evidence and meta-evidence.
...
And then, we will get rid of ``string agreement``.  Instead we need ``metaevidenceID``, ``metaevidence`` string and we have to emit ``MetaEvidence`` event.

.. literalinclude:: ../contracts/examples/SimpleEscrowWithERC1497.sol
    :language: javascript
    :lines: 1-23,25-65,67-86
...
.. literalinclude:: ../contracts/examples/SimpleEscrowWithERC1497.sol
    :language: javascript
    :lines: 1-93
    :emphasize-lines: 88-92

Congrats, now this arbitrable is ERC-1497 compatible!
